-- =============================================================================
-- PowerMind 商城库完整初始化脚本
-- 库名: power_marketplace_db
-- 警告: 会 DROP 并重建库内所有商城表；勿在 marketplace 库执行
--
-- 用法:
--   mysql -h rm-2ze7ce8lnejdjnt70xo.mysql.rds.aliyuncs.com -u DBManager -p < power_marketplace_db_full_init.sql
--
-- 执行后应看到 biz_code: PowerMind_ToC / PowerMind_Enterprise / PowerMind_Ops
-- =============================================================================

CREATE DATABASE IF NOT EXISTS `power_marketplace_db`
  DEFAULT CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;

USE `power_marketplace_db`;
...
-- ecoin_bill 分表 00000000-00000009
...
CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000001` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000001';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000002` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000002';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000003` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000003';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000004` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000004';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000005` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000005';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000006` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000006';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000007` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000007';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000008` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000008';


CREATE TABLE IF NOT EXISTS `ecoin_bill_tab_00000009` (
  `bill_id` varchar(32) NOT NULL COMMENT '业务账单ID（EB+时间+哈希+分片）',
  `user_id` bigint unsigned NOT NULL COMMENT '商城用户ID',
  `company_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '0=个人积分；>0=企业积分账户',
  `amount` decimal(20,4) NOT NULL COMMENT '预扣积分数量',
  ...
  KEY `idx_status_ctime` (`status`,`ctime`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分账单分表00000009';


-- 验证
SELECT 'biz_code_enum_tab' AS tbl, COUNT(*) AS cnt FROM biz_code_enum_tab;
SELECT code, name, scope FROM biz_code_enum_tab ORDER BY id;
SHOW TABLES;
